...
class _DocumentosPageState extends State<DocumentosPage> {
  ...
  Future<void> fetchCarpetas() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            'Error al cargar las carpetas: ${response.statusCode} - ${response.body}');
      ...
      print('Excepción al cargar las carpetas: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content:
  ...
  Future<void> fetchDocumentos(String idCarpeta) async {
    ...
    try {
      final response = await http
          .get(Uri.parse('${Config.ipback}/documentos/?id_carpeta=$idCarpeta'));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        ...
        print(
            'Error al cargar los documentos: ${response.statusCode} - ${response.body}');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
      ...
      print('Excepción al cargar los documentos: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Carpetas y Documentos (Super Admin)'),
      ...
      body: Padding(
        ...
        child: _isLoading
            ...
            : carpetasFiltradas.isEmpty && documentos.isEmpty
                ? const Center(child: Text("Sin contenido en esta carpeta."))
                : Column(
                    children: [
                      // Sección de Carpetas
